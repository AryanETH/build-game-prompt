-- Safe Matchmaking Migration (checks for existing objects)
...
BEGIN
  ...
  DROP POLICY IF EXISTS "Users can view their own matches" ON match_sessions;
  DROP POLICY IF EXISTS "Users can create match sessions" ON match_sessions;
  DROP POLICY IF EXISTS "Users can update their own matches" ON match_sessions;
  
  -- match_queue policies
  DROP POLICY IF EXISTS "Users can view match queue" ON match_queue;
  ...
  DROP POLICY IF EXISTS "Users can view game states in their matches" ON game_states;
  DROP POLICY IF EXISTS "Users can create game states" ON game_states;
END $$;
...
-- Add tables to realtime (safe to run multiple times)
DO $$
BEGIN
  -- Check and add match_sessions
  IF NOT EXISTS (

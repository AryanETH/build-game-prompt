...
export const AchievementsPanel = () => {
  const { achievements, userAchievements, unlockedCount, totalCount, getProgress } = useAchievements();
    ...
    <div className="space-y-4 px-3 md:px-0">
      ...
      <div className="flex items-center justify-between p-3 md:p-4 bg-gradient-to-r from-purple-500/10 to-blue-500/10 rounded-lg">
        ...
        <div className="text-right">
          <div className="text-2xl md:text-3xl font-bold text-yellow-500">{Math.round((unlockedCount / totalCount) * 100)}%</div>
        ...
        <TabsContent value="locked" className="mt-4">
          {lockedAchievements.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="text-sm">You're a legend!</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
